    ...
    def register():
        ...
        if request.method == 'POST':
            ...
            if errors:
                ...
                g.db.execute(
                    'INSERT INTO users (username, email, password_hash, role, full_name, phone, is_verified) VALUES (?, ?, ?, ?, ?, ?, ?)',
                    (username, email, generate_password_hash(password), role, full_name, phone, is_verified)
                )
                g.db.commit()
